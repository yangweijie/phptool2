...
final class PortKillPanel implements Panel
{
    ...
    public function build(Surface $surface, string $key, float $width, float $height): LayoutNode
    {
        ...
        $titleRow->child(LayoutNode::leaf(null, new LabelSpec('端口查杀', size: 16.0, opacity: 0.85), width: $w - 40.0, height: 36.0));
        $titleRow->child(LayoutNode::leaf(null, new LabelSpec('⭐', size: 16.0), width: 24.0, height: 36.0));

        // Search input with search icon button
        $portField = Ui::textField("{$key}:port", 'Please Input Port', $w - 44.0);
        ...
        // Search handler
        $surface->onClick("{$key}:search", function () use ($surface, $key, $portField, $w): void {
